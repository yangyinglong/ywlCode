...
void writeFile(struct PLHead * pstHead, int len);
	...
	while(sign == 1) 
	{
		...
		switch(order)
		{
			...
			case 'q':       //保存修改后的学生信息并退出系统
				writeFile(pstHead, 0);    // 这个0没有作用，自己修改函数的参数列表，去除它             
...
struct PLHead * CreatePLHead()
{
	...
	// pstNew 的 pstFirst 初始化为空
	pstNew->pstFirst = NULL;	
	return pstNew;
}
...
/**
 *对学生系统中的内容修改后进行保存
 * 这个函数就不需要len 作为参数了，第一版用数组作为容器保存学生信息的时候，我们不知道具体有多少个学生，
 * 所以一直用len作为参数来记录学生的个数，但是用了链表之后，链表里面有多少节点，就有多少学生信息，
 * 再者，在struct PLHead中有一个成员变量len，它记录了链表的长度
 */
void writeFile(struct PLHead * pstHead, int len)      //这个长度应该怎么引入
{
	...
	pstNode = pstHead -> pstFirst;
	// 1. 我们遍历链表，如果节点 == NULL，则说明链表遍历结束
	while (pstNode != NULL)
	{
		// 2. 说明节点不为空，这个节点里面的信息是一条学生信息，写入文件中
		fprintf(fpStu, "%s\t%s\t%d\t%s", pstNode -> idNumber, pstNode -> name, pstNode -> sex, pstNode -> adminName);
		// 3. 取这个节点的下一个节点
		pstNode = pstNode->pstNext;
		if (pstNode != NULL)
		{
			// 4. 如果下一个节点不为空，则在文件中写入一个换行符\n
			fprintf(fpStu,"\n");
		}
	}
